...
self.addEventListener('fetch', e => {
  ...
  if (u.origin !== location.origin) return; /* APIs always hit the network */
  if (e.request.mode === 'navigate' || u.pathname.endsWith('/index.html') || u.pathname.endsWith('/')) {
    /* network-first for the app shell: updates are picked up immediately; cache only as offline fallback */
